...
const DualLeaderboard = () => {
    ...
    const getMedalColor = (rank: number): string => {
        switch (rank) {
            ...
            default:
                return 'text-phyt_text_secondary';
        }
    };

    const getCurrentData = () => {
        if (activeBoard === 'gamblers') {
            return gamblersData[timeFrame]?.length
                ? gamblersData[timeFrame]
                : gamblersData.weekly;
        }
        return runnersData[timeFrame]?.length
            ? runnersData[timeFrame]
            : runnersData.weekly;
    };
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {(activeBoard === 'gamblers'
                    ...
                    <Card
                        key={highlight.title}
                        className="bg-phyt_form border-phyt_form_border"
                    >
                        <CardContent className="p-6">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-phyt_text mt-1">
                                        {highlight.value}
                                    </p>
                                    <p className="text-phyt_blue text-sm mt-1">
                                        {highlight.username}
                                    </p>
                                </div>
            ...
            <div className="flex space-x-4 border-b border-phyt_form">
                {['weekly', 'monthly', 'allTime'].map((period) => (
                    <button
                        ...
                        onClick={() => {
                            setTimeFrame(period as TimeFrame);
                        }}
                    >
